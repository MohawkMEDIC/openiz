...
namespace OpenIZ.Core.PCL.Test
{
    ...
    public class SimplePatchServiceTest
    {
        ...
        public void DiffShouldGenerateForSubMembers()
        {
            ...
            Assert.IsNotNull(patch);
            Assert.AreEqual(9, patch.Operation.Count);
            Assert.AreEqual(PatchOperationType.Add, patch.Operation[5].OperationType);
            Assert.AreEqual(b.Identifiers[1], patch.Operation[5].Value);
            Assert.AreEqual("identifier", patch.Operation[5].Path);

            using (StringWriter sw = new StringWriter())
            {
                var xsz = new XmlSerializer(typeof(Patch));
                xsz.Serialize(sw, patch);
                var patchXml = sw.ToString();
            }

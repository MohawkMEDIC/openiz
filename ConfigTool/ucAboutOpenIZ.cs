...
{
    public partial class ucAboutOpenIZ : UserControl
    {
        public ucAboutOpenIZ()
        {
            InitializeComponent();
        ...
        {
            string exeFile = Path.Combine(Path.GetDirectoryName(Assembly.GetEntryAssembly().Location), "OpenIZ.exe");
            if (!File.Exists(exeFile))
                lblVersion.Text = rtfLicense.Text = lblCopyright.Text = "Missing OpenIZ.exe";
            var asm = Assembly.LoadFile(exeFile);

            // Get asm attributes
            lblVersion.Text = $"{asm.GetName().Version.ToString()} ({asm.GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion})";

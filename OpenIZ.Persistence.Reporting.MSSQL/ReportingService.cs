...
using OpenIZ.Persistence.Reporting.MSSQL.Configuration;

namespace OpenIZ.Persistence.Reporting.MSSQL
{
	/// <summary>
	...
	public class ReportingService : IDaemonService
	{
		...
		/// </summary>
		private readonly TraceSource traceSource = new TraceSource("OpenIZ.Persistence.Reporting.MSSQL");

		static ReportingService()
		{
			var tracer = new TraceSource("OpenIZ.Persistence.Reporting.MSSQL");
			try
			{
				ModelMapper = new ModelMapper(typeof(ReportingService).GetTypeInfo().Assembly.GetManifestResourceStream("OpenIZ.Persistence.Reporting.MSSQL.Data.ModelMap.xml"));
			}
		...
		public static ReportingConfiguration Configuration
		{
			get
			{
				...
				{
					config = ConfigurationManager.GetSection("openiz.persistence.reporting.mssql") as ReportingConfiguration;
				}
				else
				{
					config = configurationManager.GetSection("openiz.persistence.reporting.mssql") as ReportingConfiguration;
		...
		public bool Start()
		{
			...
			try
			{
				...
				foreach (var t in typeof(ReportingService).GetTypeInfo().Assembly.DefinedTypes.Where(o => o.Namespace == "OpenIZ.Persistence.Reporting.MSSQL.Services" && !o.GetTypeInfo().IsAbstract))
				{
					try
					{

...
namespace OpenIZ.Persistence.Reporting.MSSQL.Services
{
	...
	/// </summary>
	internal class ReportFormatPersistenceService : ReportPersistenceServiceBase<ReportFormat, MSSQL.Model.ReportFormat>, IDataPersistenceService<ReportFormat>
	{
		/// <summary>
		...
		public IEnumerable<ReportFormat> Query(Expression<Func<ReportFormat, bool>> query, int offset, int? count, IPrincipal authContext, out int totalCount)
		{
			...
			using (var context = new ApplicationDbContext())
			{
				var expression = ModelMapper.MapModelExpression<ReportFormat, MSSQL.Model.ReportFormat>(query);
		...
		internal override MSSQL.Model.ReportFormat FromModelInstance(ReportFormat modelInstance)
		{
			this.tracer.TraceEvent(TraceEventType.Verbose, 0, $"Mapping { nameof(MSSQL.Model.ReportFormat) } to { nameof(ReportFormat) }");
			return modelInstance == null ? null : ModelMapper.MapModelInstance<ReportFormat, MSSQL.Model.ReportFormat>(modelInstance);
		}
		...
		/// <returns>Returns the converted model instance.</returns>
		internal override ReportFormat ToModelInstance(MSSQL.Model.ReportFormat domainInstance)
		{
			this.tracer.TraceEvent(TraceEventType.Verbose, 0, $"Mapping { nameof(ReportFormat) } to { nameof(MSSQL.Model.ReportFormat) }");
			return domainInstance == null ? null : ModelMapper.MapDomainInstance<MSSQL.Model.ReportFormat, ReportFormat>(domainInstance);
		}
	}
}
...
namespace OpenIZ.Persistence.Reporting.MSSQL.Services
{
	...
	/// </summary>
	internal class ParameterTypePersistenceService : ReportPersistenceServiceBase<ParameterType, MSSQL.Model.ParameterType>, IDataPersistenceService<ParameterType>
	{
		/// <summary>
		...
		public IEnumerable<ParameterType> Query(Expression<Func<ParameterType, bool>> query, int offset, int? count, IPrincipal authContext, out int totalCount)
		{
			...
			using (var context = new ApplicationDbContext())
			{
				var expression = ModelMapper.MapModelExpression<ParameterType, MSSQL.Model.ParameterType>(query);
		...
		internal override MSSQL.Model.ParameterType FromModelInstance(ParameterType modelInstance)
		{
			this.tracer.TraceEvent(TraceEventType.Verbose, 0, $"Mapping { nameof(MSSQL.Model.ParameterType) } to { nameof(ParameterType) }");
			return modelInstance == null ? null : ModelMapper.MapModelInstance<ParameterType, MSSQL.Model.ParameterType>(modelInstance);
		}
		...
		/// <returns>Returns the converted model instance.</returns>
		internal override ParameterType ToModelInstance(MSSQL.Model.ParameterType domainInstance)
		{
			this.tracer.TraceEvent(TraceEventType.Verbose, 0, $"Mapping { nameof(ParameterType) } to { nameof(MSSQL.Model.ParameterType) }");
			return domainInstance == null ? null : ModelMapper.MapDomainInstance<MSSQL.Model.ParameterType, ParameterType>(domainInstance);
		}
	}
}
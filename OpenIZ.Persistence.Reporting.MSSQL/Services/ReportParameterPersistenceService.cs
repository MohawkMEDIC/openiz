...
namespace OpenIZ.Persistence.Reporting.MSSQL.Services
{
	...
	/// </summary>
	internal class ReportParameterPersistenceService : ReportPersistenceServiceBase<ReportParameter, MSSQL.Model.ReportParameter>, IDataPersistenceService<ReportParameter>
	{
		/// <summary>
		...
		public IEnumerable<ReportParameter> Query(Expression<Func<ReportParameter, bool>> query, int offset, int? count, IPrincipal authContext, out int totalCount)
		{
			...
			using (var context = new ApplicationDbContext())
			{
				var expression = ModelMapper.MapModelExpression<ReportParameter, MSSQL.Model.ReportParameter>(query);
		...
		internal override MSSQL.Model.ReportParameter FromModelInstance(ReportParameter modelInstance)
		{
			this.tracer.TraceEvent(TraceEventType.Verbose, 0, $"Mapping { nameof(MSSQL.Model.ReportParameter) } to { nameof(ReportParameter) }");
			return modelInstance == null ? null : ModelMapper.MapModelInstance<ReportParameter, MSSQL.Model.ReportParameter>(modelInstance);
		}
		...
		/// <returns>Returns the converted model instance.</returns>
		internal override ReportParameter ToModelInstance(MSSQL.Model.ReportParameter domainInstance)
		{
			this.tracer.TraceEvent(TraceEventType.Verbose, 0, $"Mapping { nameof(ReportParameter) } to { nameof(MSSQL.Model.ReportParameter) }");
			return domainInstance == null ? null : ModelMapper.MapDomainInstance<MSSQL.Model.ReportParameter, ReportParameter>(domainInstance);
		}
	}
}
...
namespace OpenIZ.Persistence.Data.ADO.Security
{
    ...
    public class SqlClaimsIdentity : IIdentity
    {
            ...
            try
            {
                ...
                using (var dataContext = s_configuration.Provider.GetWriteConnection())
                {
                    ...
                    var roles = dataContext.Query<DbSecurityRole>(new SqlStatement<DbSecurityRole>().SelectFrom()
                        .InnerJoin<DbSecurityUserRole, DbSecurityUserRole>()
                        .Where(o => o.UserKey == user.Key));

                    var userIdentity = new SqlClaimsIdentity(user, roles, true) { m_authenticationType = "Password" };
        ...
        /// Private ctor
        /// </summary>
        private SqlClaimsIdentity(DbSecurityUser user, IEnumerable<DbSecurityRole> roles,  bool isAuthenticated)
        {

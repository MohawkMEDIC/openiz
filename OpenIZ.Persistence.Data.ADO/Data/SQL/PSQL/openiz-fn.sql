...
CREATE OR REPLACE FUNCTION AUTH_USR (
	USR_NAME_IN IN VARCHAR(128),
	PASSWD_IN IN VARCHAR(128),
	MAX_FAIL_LOGIN_IN IN INT
) RETURNS SETOF SEC_USR_TBL AS $$
DECLARE
	USR_TPL SEC_USR_TBL;
BEGIN
	...
	ELSE
		...
		ELSIF (USR_TPL.TFA_ENABLED) THEN
			UPDATE SEC_USR_TBL SET FAIL_LOGIN = FAIL_LOGIN + 1 WHERE USR_NAME = USR_NAME_IN;
			RAISE EXCEPTION 'User must use two factor authentication';
		ELSIF (USR_TPL.PASSWD = PASSWD_IN) THEN
			...
			WHERE USR_NAME = USR_NAME_IN;
			RETURN QUERY (SELECT * FROM USR_TPL);
		ELSE
			UPDATE SEC_USR_TBL SET FAIL_LOGIN = FAIL_LOGIN + 1 WHERE USR_NAME = USR_NAME_IN;
...
CREATE OR REPLACE FUNCTION AUTH_APP (
	APP_PUB_ID_IN IN VARCHAR(128),
	APP_SCRT_IN IN VARCHAR(128)
...
CREATE OR REPLACE FUNCTION AUTH_DEV (
	DEV_PUB_ID_IN IN VARCHAR(128),
	DEV_SCRT_IN IN VARCHAR(128)

...
BEGIN
	...
	IF (IS_USR_LOCK(USR_NAME_IN)) THEN
		UPDATE SEC_USR_TBL SET FAIL_LOGIN = SEC_USR_TBL.FAIL_LOGIN + 1, LOCKED = CURRENT_TIMESTAMP + ((SEC_USR_TBL.FAIL_LOGIN - MAX_FAIL_LOGIN_IN) * '30 SECONDS'::INTERVAL)
			WHERE SEC_USR_TBL.USR_NAME = USR_NAME_IN;
		ERR_CODE = 'ERR:ACCOUNT LOCKED OUT'::CITEXT;
	ELSE
		...
		ELSIF (USR_TPL.TFA_ENABLED) THEN
			...
			ERR_CODE = 'ERR:USER REQUIRES TWO-FACTOR AUTHENTICATION'::CITEXT;
		ELSIF (USR_TPL.PASSWD = PASSWD_IN) THEN
			...
			WHERE SEC_USR_TBL.USR_NAME = USR_NAME_IN;
			RETURN QUERY SELECT USR_TPL.*, NULL;
		ELSE
			UPDATE SEC_USR_TBL SET FAIL_LOGIN = USR_TPL.FAIL_LOGIN + 1 WHERE SEC_USR_TBL.USR_NAME = USR_NAME_IN;
			ERR_CODE = 'ERR:INVALID USERNAME/PASSWORD'::CITEXT;
		END IF;
	END IF;
	RETURN QUERY SELECT USR_TPL.*, ERR_CODE;
END	
$$ LANGUAGE PLPGSQL;

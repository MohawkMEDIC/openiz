...
namespace MARC.HI.EHRS.SVC.Presentation.Configuration
{
    ...
    public class ServiceConfigurationPanel : IConfigurableFeature
    {
        ...
        public void Configure(System.Xml.XmlDocument configurationDom)
        {
            ...
            try
            {
                ServiceTools.ServiceInstaller.Install("OpenIZ Host Process", "Host Process for OpenIZ", Path.Combine(Path.GetDirectoryName(Assembly.GetEntryAssembly().Location), "OpenIZ.exe"), this.AccountName, this.AccountPassword == null ? null : this.AccountPassword.ToString(), this.Mode);
            }
            catch (Exception e)
            {
        ...
        {
            bool isInstalled = ServiceTools.ServiceInstaller.ServiceIsInstalled("OpenIZ Host Process");
            if (isInstalled)
            {
                var svcInfo = ServiceTools.ServiceInstaller.GetServiceConfig("OpenIZ Host Process");
                if (svcInfo != null)
                {
        ...
        public string Name
        {
            get { return "OpenIZ/Windows Service"; }
        }

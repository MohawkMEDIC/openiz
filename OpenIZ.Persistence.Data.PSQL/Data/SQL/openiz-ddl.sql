...
CREATE TABLE PHON_VAL_TBL (
	PHON_VAL_ID UUID NOT NULL DEFAULT uuid_generate_v4(), -- UNIQUE IDENTIFIER FORHTE VALUE
...
CREATE UNIQUE INDEX CD_VRSN_MNEMONIC_UQ_IDX ON CD_VRSN_TBL(MNEMONIC) WHERE OBSLT_UTC IS NULL;
...
$$
BEGIN
	RETURN (SELECT COUNT(*) FROM CD_VRSN_TBL INNER JOIN CD_CLS_TBL USING (CLS_ID) WHERE CD_ID = CD_ID_IN AND CD_CLS_TBL.MNEMONIC = CLS_MNEMONIC_IN) > 0;
END;
$$ LANGUAGE PLPGSQL;
...
$$
BEGIN
	RETURN (SELECT COUNT(*) FROM CD_SET_MEM_ASSOC_TBL INNER JOIN CD_SET_TBL USING(SET_ID) WHERE CD_ID = CD_ID_IN AND MNEMONIC = SET_MNEMONIC_IN) > 0;
END;
$$ LANGUAGE PLPGSQL;
...
CREATE TABLE EXT_TYP_TBL (
	...
	IS_ACTIVE BOOLEAN NOT NULL DEFAULT FALSE, -- TRUE IF THE EXTENSION REGISTRATION IS ACTIVE
...
CREATE TABLE TPL_DEF_TBL (
	...
	OID VARCHAR(64) NOT NULL, -- THE OID OF THE TEMPLATE DEFINITION
...
CREATE TABLE ACT_VRSN_TBL (
	...
	RSN_CD_ID UUID NOT NULL, -- THE REASON OF THE ACT TO OCCUR
...
CREATE TABLE ACT_ID_TBL (
	...
	ID_TYP_ID UUID NOT NULL, -- THE TYPE OF IDENTIFIER 
...
CREATE TABLE OBS_TBL (
	...
	VAL_TYP CHAR(2) NOT NULL CHECK (VAL_TYP IN ('PQ','ST','CD')), -- THE TYPE OF VALUE
...
CREATE TABLE QTY_OBS_TBL (
	...
	QTY_PRC NUMERIC(2) NOT NULL, -- THE PRECISION OF THE OBSERVED VALUES
	...
	CONSTRAINT CK_QTY_OBS_UOM_CD CHECK (ASSRT_CD_CLS(UOM_CD_ID, 'UnitOfMesaure'))
);
...
CREATE TABLE SUB_ADM_TBL (
	...
	RTE_CD_ID UUID NOT NULL, -- THE ROUTE OF ADMINISTRATION
...
CREATE TABLE ACT_POL_ASSOC_TBL (
	POL_INST_ID UUID NOT NULL DEFAULT uuid_generate_v4(),
	...
	CONSTRAINT PK_ACT_POL_ASSOC_TBL PRIMARY KEY(POL_INST_ID),
...
CREATE TABLE ENT_TBL (
	...
	TPL_ID UUID NOT NULL, -- THE TEMPLATE WHICH THIS ENTITY FOLLOWS
...
CREATE TABLE ENT_TAG_TBL (
	ENT_TAG_ID UUID NOT NULL DEFAULT uuid_generate_v4(),
	...
	CONSTRAINT PK_ENT_TAG_TBL PRIMARY KEY (ENT_TAG_ID),
...
CREATE TABLE ENT_REL_TBL (
	...
	QTY INT NOT NULL DEFAULT 1, -- THE NUMER OF THE TARGET ENTITY RELATED TO THE SOURCE ENTIYT
...
-- INDEX FO ADDRESS
CREATE INDEX ENT_ADDR_ENT_ID_IDX ON ENT_ADDR_TBL(ADDR_ID);
...
CREATE UNIQUE INDEX EN_ADDR_CMP_VAL_VAL_IDX ON ENT_ADDR_CMP_VAL_TBL(VAL);

-- ENTITY ADDRESS COMPONENT TABLE
CREATE TABLE ENT_ADDR_CMP_TBL (
	CMP_ID UUID NOT NULL DEFAULT uuid_generate_v4(),
	TYP_CD_ID UUID NOT NULL, -- THE TYPE OF THE COMPONENT (STREET, CITY, ETC.)
	...
	CONSTRAINT CK_ENT_ADDR_CMP_TYP_CD CHECK (IS_CD_SET_MEM(TYP_CD_ID, 'AddressComponentType'))
);
...
CREATE TABLE ENT_NAME_CMP_TBL (
	...
	TYP_CD_ID UUID NOT NULL, -- THE TYPE OF COMPONENT (FAMILY, GIVEN, ETC);
	PHON_VAL_ID UUID NOT NULL, -- THE PHONETIC CODE FOR THE NAME
	NAME_ID UUID NOT NULL, -- THE NAME TO WHICH THE COMPONENT APPLIES
	CONSTRAINT PK_ENT_NAME_CMP_TBL PRIMARY KEY (NAME_CMP_ID),
	CONSTRAINT FK_ENT_NAME_CMP_TYP_CD_ID FOREIGN KEY (TYP_CD_ID) REFERENCES CD_TBL(CD_ID),
	CONSTRAINT FK_ENT_NAME_CMP_PHON_VAL_ID FOREIGN KEY (PHON_VAL_ID) REFERENCES PHON_VAL_TBL(PHON_VAL_ID),
	CONSTRAINT FK_ENT_NAME_CMP_NAME_ID FOREIGN KEY (NAME_ID) REFERENCES ENT_NAME_TBL(NAME_ID),
	CONSTRAINT CK_ENT_NAME_CMP_TYP_CD CHECK (IS_CD_SET_MEM(TYP_CD_ID, 'NameComponentType'))
);

CREATE INDEX ENT_NAME_CMP_NAME_ID_IDX ON ENT_NAME_CMP_TBL(NAME_ID);
CREATE INDEX ENT_NAME_CMP_PHON_VAL_ID_IDX ON ENT_NAME_CMP_TBL(PHON_VAL_ID);
...
CREATE TABLE ENT_EXT_TBL (
	...
	CONSTRAINT FK_ENT_EXT_TYP_ID FOREIGN KEY (EXT_TYP_ID) REFERENCES CD_TBL(CD_ID),
	...
	CONSTRAINT CK_ENT_EXT_TYP_ID FOREIGN KEY (EXT_TYP_ID) REFERENCES EXT_TYP_TBL(EXT_TYP_ID)
);
...
CREATE TABLE ENT_ID_TBL (
	...
	ID_TYP_ID UUID NOT NULL, -- THE TYPE OF IDENTIIFER
...
CREATE TABLE ENT_TEL_TBL (
	...
	CONSTRAINT CK_ENT_TEL_TYP_CD CHECK (IS_CD_SET_MEM(TYP_CD_ID, 'TelecomAddressType')),
	CONSTRAINT CK_ENT_TEL_USE_CD CHECK (IS_CD_SET_MEM(USE_CD_ID, 'TelecomAdressUse'))
);
...
CREATE TABLE ORG_TBL (
	...
	CONSTRAINT CK_ORG_IND_CD CHECK (IS_CD_SET_MEM(IND_CD_ID, 'IndustryCode'))
);

-- APPLICATION ENTITY
CREATE TABLE APP_ENT_TBL (
	ENT_VRSN_ID UUID NOT NULL, -- THE ENTITY TO WHICH THE VERISON APPLIES
	SEC_APP_ID UUID NOT NULL, -- THE SECURITY APPLICATION ASSOCIATION EDITH THE ENTITY
...
CREATE TABLE DEV_ENT_TBL (
	...
	SEC_DEV_ID UUID NOT NULL, -- THE SECURITY DEVICE TO WHICH THE DEVICE INFORMATION APPLIES
...
CREATE TABLE MAT_TBL (
	...
	CONSTRAINT CK_MAT_FRM_CD CHECK (ASSRT_CD_CLS(FRM_CD_ID, 'Form')),
	CONSTRAINT CK_MAT_QTY_CD CHECK (ASSRT_CD_CLS(QTY_CD_ID, 'UnitOfMeasure'))
);
...
CREATE TABLE PSN_LNG_TBL (
	...
	LANG_CS CHAR(2) NOT NULL, -- THE LANGUAGE CODE REPRESENTING THE COMMUNICATION
...
CREATE TABLE ACT_PTCPT_TBL (
	...
	QTY INT NOT NULL DEFAULT 1, -- THE QUANTITY OF ENTITY PARTICIPATING IN ACT
